...
extension FormatConverter {
    ...
    func convertPCMToCompressed(completionHandler: FormatConverterCallback? = nil) {
        ...
        case "wav":
            ...
            formatKey = kAudioFormatLinearPCM
        default:
            let err = TSPlayerModuleError.audiokitError("Unsupported output format: \(outputFormat)")
            print(err)
            return
        }
        ...
        if sampleRate == 0 {
            let err = TSPlayerModuleError.audiokitError("Sample rate can't be 0 - assigning to default format of 48k. inputFormat is \(inputFormat)")
            print(err)
        ...
        if !writer.startWriting() {
            let err = TSPlayerModuleError.audiokitError("Failed to start writing. Error: \(writer.error?.localizedDescription ?? "")")
            print(err)
        ...
        if !reader.startReading() {
            let err = TSPlayerModuleError.audiokitError("Failed to start reading. Error: \(reader.error?.localizedDescription)")
            print(err)
            ...
            while writerInput.isReadyForMoreMediaData, processing {
                if reader.status == .reading,
                   let buffer = readerOutput.copyNextSampleBuffer() {
                    ...
                    case .failed:
                        let err = TSPlayerModuleError.audiokitError("Conversion failed with error: \(reader.error)")
                        print(err)
                        writer.cancelWriting()
                        completionHandler?(reader.error)
                    case .cancelled:
                        let err = TSPlayerModuleError.audiokitError("Conversion cancelled")
                        print(err)
