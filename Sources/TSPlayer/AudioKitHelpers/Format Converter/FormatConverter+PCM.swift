...
extension FormatConverter {
    func convertToPCM(completionHandler: FormatConverterCallback? = nil) {
        ...
        func closeFiles() {
            if let strongFile = inputFile {
                ...
                if noErr != ExtAudioFileDispose(strongFile) {
                    let err = TSPlayerModuleError.audiokitError("Error disposing input file, could have a memory leak")
                    print(err)
                }
            }
            inputFile = nil
            ...
            if let strongFile = outputFile {
                ...
                if noErr != ExtAudioFileDispose(strongFile) {
                    let err = TSPlayerModuleError.audiokitError("Error disposing output file, could have a memory leak")
                    print(err)
                }
            }
            outputFile = nil
        ...
        guard inputFormat != outputFormat ||
            ...
            let err = TSPlayerModuleError.audiokitError("No conversion is needed, formats are the same. Copying to \(outputURL)")
            print(err)
            ...
            do {
                ...
                let error = TSPlayerModuleError.audiokitError(err.localizedDescription)
                print(error)
            }
            return
        }
